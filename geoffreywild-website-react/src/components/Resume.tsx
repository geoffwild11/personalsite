...
function Resume() {
    ...
    return (<div>
        ...
        {workExpanded 
            ? <div className="collapsibleSection">{jobs.map((job: IJobInfo) => <Job jobInfo={job}/>)}</div> 
            : <div />
        }
        ...
        {educationExpanded 
            ? <div className="collapsibleSection">{education.map((eduInfo: IEducationInfo) => <Education educationInfo={eduInfo}/>)}</div> 
            : null
        }
    </div>
    );
